        ...
        {quest.bids.length > 0 && isCreator && (
            <div>
                 ...
                 <div className="space-y-2">
                    {quest.bids.map(bid => (
                        <div key={bid.userId} className="bg-gray-800/50 p-3 rounded-lg flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                <img src={bid.avatarUrl} alt={bid.username} className="w-10 h-10 rounded-full" />
                                <div>
                                    <p className="font-semibold">{bid.username}</p>
                                    <p className="text-xs text-gray-400">Bid placed {formatTimeLeft(bid.timestamp)}</p>
                                </div>
                            </div>
